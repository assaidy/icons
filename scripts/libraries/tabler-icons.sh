...
if [[ "$CURRENT" == "$LATEST" ]]; then
  echo "tabler-icons: no changes (${CURRENT})"
  exit 0
fi

TMP=$(download_gh_release "$REPO" "$LATEST" "icons/outline")
...
if diff_svgs "$PKG_DIR" "$TMP/svgs"; then
  log "tabler-icons: same SVGs, only version bump"
  ...
  echo "tabler-icons: bumped $CURRENT → $LATEST (${NEW_COUNT} icons)"
else
  echo "tabler-icons: $CURRENT → $LATEST"
